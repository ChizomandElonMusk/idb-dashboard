...
/* harmony default export */ var myto_dashboardvue_type_script_lang_js = ({
  ...
  methods: {
    async getData() {
      ...
      this.error = null;
      try {
        const myto = await controlCenterApi["l" /* getMytoDashboard */]();
        const totalDtEnergy = Object(controlCenterApi["r" /* pick */])(myto, ['summary.total_dt_energy_raw'], null);
        this.total_consumption = totalDtEnergy != null ? Object(controlCenterApi["b" /* formatNumber */])(totalDtEnergy) : '0';
        const monthStart = Object(controlCenterApi["r" /* pick */])(myto, ['month', 'summary.month_start'], null);
        ...
        const bandSummary = Object(controlCenterApi["r" /* pick */])(myto, ['band_summary'], []) || [];
    ...
    buildBands(bandSummary, totalEnergy) {
      const denom = totalEnergy || bandSummary.reduce((sum, b) => sum + (Number(Object(controlCenterApi["r" /* pick */])(b, ['dt_energy_raw'], 0)) || 0), 0) || 1;
      return bandSummary.map(b => {
        const energy = Number(Object(controlCenterApi["r" /* pick */])(b, ['dt_energy_raw'], 0)) || 0;
        const totalDts = Object(controlCenterApi["r" /* pick */])(b, ['total_dts'], null);
        const count = totalDts != null ? Object(controlCenterApi["b" /* formatNumber */])(totalDts) : '—';
        const pct = denom ? energy / denom * 100 : 0;
        return {
          name: `Band ${Object(controlCenterApi["r" /* pick */])(b, ['band_code'], '—')}`,
          pct: `${pct.toFixed(2)}%`,
          count,
          energy
        };
    ...
    buildBandAvailability(bandSummary) {
      return bandSummary.map(b => {
        const hours = Object(controlCenterApi["r" /* pick */])(b, ['avg_dt_availability_hours', 'dt_availability_hours', 'avg_availability_hours'], null);
        return hours != null ? {
          name: `Band ${Object(controlCenterApi["r" /* pick */])(b, ['band_code'], '—')}`,
    ...
    async loadTrend(baseMonth) {
      const months = Object(controlCenterApi["p" /* lastNMonths */])(baseMonth, 6);
      const responses = await Promise.all(months.map(m => controlCenterApi["l" /* getMytoDashboard */]({
        month: m
      }).catch(() => null)));
      this.renderTrendChart(months.map(controlCenterApi["q" /* monthLabel */]), responses.map(r => Object(controlCenterApi["r" /* pick */])(r, ['summary.total_dt_energy_raw'], null)));
    },
...
installComponents(component, {SideNav: __webpack_require__(41).default,PreLoader: __webpack_require__(49).default,AnimatedValue: __webpack_require__(46).default,ChartPie: __webpack_require__(57).default,CertificationBadge: __webpack_require__(52).default})
